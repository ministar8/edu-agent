...
export function ConversationList({ activeThreadId, onSelect, onNewChat, refreshKey }: ConversationListProps) {
  ...
  const getBaseThread = (threadId: string) => {
    const parts = threadId.split(":");
    return parts.length > 1 ? parts.slice(1).join(":") : threadId;
  };
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto px-3 space-y-2">
        {loading ? (
          ...
          <div className="text-xs text-slate-400 text-center py-4">暂无对话</div>
        ) : (
          conversations.map((conv) => {
            const isActive = getBaseThread(conv.thread_id) === activeThreadId || conv.thread_id === activeThreadId;
            ...
            return (
              <div key={conv.id} onClick={() => onSelect(conv)} className={"group relative flex h-12 w-full cursor-pointer items-center rounded-xl border px-3 text-left transition-colors " + (isActive ? "border-emerald-200 bg-emerald-50" : "border-transparent hover:bg-stone-50")}>
                ...
                <button onClick={(e) => handleDelete(e, conv.id)} disabled={isDeleting} className="absolute right-2 top-1/2 -translate-y-1/2 rounded p-1 text-slate-300 opacity-0 transition-opacity hover:bg-red-50 hover:text-red-500 group-hover:opacity-100 disabled:opacity-50" title="删除">
                  {isDeleting ? (
