...
- a1是**表头元素**；an是**表尾元素**。
- 线性表中的位序是**从 1 开始** 的**，而** 数组**中元素的下标是** 从 0开始的。
 ...
 **⚠ 注意**：链式存储，结点内的** 存储单元地址**是一定连续的**，但是** 存储空间不一定连续。
...
- **逆置**：将线性表中元素顺序反转的操作，如(a1,a2,...,an)变为(an,...,a2,a1)，可通过头尾交换实现
- **线性表基本操作**：InitList初始化、DestroyList销毁、ListInsert插入、ListDelete删除、LocateElem按值查找、GetElem按位查找、Length求表长、Empty判空
