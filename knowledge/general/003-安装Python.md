...
方法一，可以直接从Python的官方网站下载Python 3对应的
Windows安装程序
，推荐下载
...
运行Python
注意
Python在Linux/macOS的命令是
python3
，在Windows下的命令是
python
，后续请自行根据操作系统选择合适的命令。
安装成功后，打开命令行窗口（Windows下打开PowerShell），敲入
python
后，会出现两种情况：
情况一：
┌─────────────────────────────────────────────────────────┐
...
在Windows上运行Python时，请先启动PowerShell命令行，然后运行
python
；
在macOS和Linux上运行Python时，请打开终端，然后运行
python3
；
退出Python交互式环境，需要在提示符
>>>
下输入
exit()
并回车确认。
