...
def _check_score_quality(fused: FusedEvidence) -> CheckResult:
    ...
    low_count = sum(1 for s in scores if s < _MIN_RERANK_SCORE)

    if avg_score < _MIN_AVG_SCORE:
        return CheckResult(name="score_quality", passed=False, score=avg_score,
                            detail=f"平均分过低: {avg_score:.3f} < {_MIN_AVG_SCORE}")

    # 低分占比惩罚
    low_ratio = low_count / len(scores)
    score = avg_score * (1.0 - 0.3 * low_ratio)
...
def verify_evidence(
    ...
    if use_llm and query:
        llm_result = _run_llm_relevance_check(query, fused)
        if llm_result is not None:
            checks.append(llm_result)

    # 3. 计算综合分数
    active_weights = dict(_CHECK_WEIGHTS)
    # 如果 LLM 层未执行，将其权重按比例分配给其他项
