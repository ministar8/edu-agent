...
def exact_dedup(documents: list[Document]) -> tuple[list[Document], list[dict]]:
    ...
    for doc in documents:
        ...
        source = str(
            doc.metadata.get("source_path") or doc.metadata.get("source_file") or "unknown"
        )
        ...
        if content_hash in seen_hashes:
            ...
            logger.debug(
                "Exact dedup: '%s' is duplicate of '%s'", source, seen_hashes[content_hash]
            )
        else:
...
def fuzzy_dedup(
    ...
    if use_lsh:
        try:
            from datasketch import MinHashLSH  # noqa: F401 — 可用性探测
