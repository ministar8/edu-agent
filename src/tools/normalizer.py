...
_FULLWIDTH_TO_HALFWIDTH_TABLE = str.maketrans(
    {chr(cp): chr(cp - _FW_OFFSET) for cp in range(0xFF10, 0xFF1A + 1)}  # 全角数字 ０-９
    | {chr(cp): chr(cp - _FW_OFFSET) for cp in range(0xFF21, 0xFF3B + 1)}  # 全角大写 Ａ-Ｚ
    | {chr(cp): chr(cp - _FW_OFFSET) for cp in range(0xFF41, 0xFF5B + 1)}  # 全角小写 ａ-ｚ
)
...
_DATE_SLASH_RE = re.compile(r"(\d{4})\s*[/\-.]\s*(\d{1,2})\s*[/\-.]\s*(\d{1,2})")
# "2024.1" → "2024-01" (年月简写)
_DATE_DOT_MONTH_RE = re.compile(r"(?<!\d)(\d{4})\s*\.\s*(\d{1,2})\s*\.?(?!\d)")

# ── 时间格式统一 ──
# "１２：３０" / "12点30分" → "12:30"
_TIME_CN_RE = re.compile(r"(\d{1,2})\s*[点時]\s*(\d{1,2})\s*分?")
# "12时30分" → "12:30"
_TIME_SHI_RE = re.compile(r"(\d{1,2})\s*时\s*(\d{1,2})\s*分?")
