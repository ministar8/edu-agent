...
class _ResourceGuard:
    ...
    def degrade(self, operation: str) -> None:
        ...
        if operation not in self._degraded:
            ...
            logger.warning(
                "ResourceGuard: '%s' degraded (calls=%d, limit=%d)",
                operation,
                self._counters.get(operation, 0),
...
def _with_timeout(timeout: float):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if elapsed > timeout:
                logger.warning(
                    "Timeout: %s took %.1fs (limit=%.1fs)", func.__name__, elapsed, timeout
                )
                return None
            return result
...
_SEGMENT_PROFILES: dict[str, _SegmentProfile] = {
    # ── 法律/合同：段落长、主题稳定，少分割 ──
    "law": _SegmentProfile(
        window_size=80, stride=40, diff_threshold=0.65, min_chunk=800, max_chunk=3000
    ),
    "contract": _SegmentProfile(
        window_size=80, stride=40, diff_threshold=0.65, min_chunk=800, max_chunk=3000
    ),
    "regulation": _SegmentProfile(
        window_size=80, stride=40, diff_threshold=0.65, min_chunk=800, max_chunk=3000
    ),
    # ── 技术文档：段落短、主题切换快，多分割 ──
    "tech": _SegmentProfile(
        window_size=30, stride=15, diff_threshold=0.45, min_chunk=300, max_chunk=1500
    ),
    "api": _SegmentProfile(
        window_size=30, stride=15, diff_threshold=0.45, min_chunk=300, max_chunk=1500
    ),
    "code": _SegmentProfile(
        window_size=30, stride=15, diff_threshold=0.45, min_chunk=300, max_chunk=1500
    ),
    "tutorial": _SegmentProfile(
        window_size=40, stride=20, diff_threshold=0.50, min_chunk=400, max_chunk=1500
    ),
    # ── 小说/文学：连续叙事，少分割 ──
    "novel": _SegmentProfile(
        window_size=100, stride=50, diff_threshold=0.70, min_chunk=1000, max_chunk=4000
    ),
    "fiction": _SegmentProfile(
        window_size=100, stride=50, diff_threshold=0.70, min_chunk=1000, max_chunk=4000
    ),
    "literature": _SegmentProfile(
        window_size=100, stride=50, diff_threshold=0.70, min_chunk=1000, max_chunk=4000
    ),
    # ── 新闻/论文：段落分明 ──
    "news": _SegmentProfile(
        window_size=50, stride=25, diff_threshold=0.55, min_chunk=500, max_chunk=2000
    ),
    "paper": _SegmentProfile(
        window_size=50, stride=25, diff_threshold=0.55, min_chunk=500, max_chunk=2000
    ),
    "thesis": _SegmentProfile(
        window_size=50, stride=25, diff_threshold=0.55, min_chunk=500, max_chunk=2000
    ),
    # ── 教材/讲义 ──
    "textbook": _SegmentProfile(
        window_size=60, stride=30, diff_threshold=0.55, min_chunk=600, max_chunk=2500
    ),
    "lecture": _SegmentProfile(
        window_size=60, stride=30, diff_threshold=0.55, min_chunk=600, max_chunk=2500
    ),
    # ── 默认兜底 ──
    "default": _SegmentProfile(
        window_size=50, stride=25, diff_threshold=0.55, min_chunk=500, max_chunk=2000
    ),
}
...
def _extract_pdf_heading_by_features(text: str) -> str | None:
    ...
    for i, line in enumerate(search_lines):
        ...
        if 2 <= len(stripped) <= 40 and _NO_END_PUNCT_RE.search(stripped) and prev_blank:
            # 检查下一行是否为空行（独立行特征）
...
def _detect_pdf_title_lines(text: str) -> list[dict]:
    ...
    for i, line in enumerate(lines):
        ...
        # 规则3：短行 + 无终结标点 + 独立行
        elif 2 <= len(stripped) <= 40 and _NO_END_PUNCT_RE.search(stripped) and prev_blank:
            # 检查下一行是否为空行
...
def _impute_single_doc(doc: Document) -> tuple[Document, list[dict]]:
    ...
    if heading_result is not None:
        ...
        doc.metadata["has_heading"] = bool(
            heading_filled
            and heading_filled != "[未命名文档]"
            and not heading_filled.startswith(_LOW_CONFIDENCE_PREFIX)
        )
        doc_log.append(
            {
                "field": "heading",
...
def impute_documents(
    ...
    if parallel and len(documents) > 10:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = {
                executor.submit(_impute_single_doc, doc): i for i, doc in enumerate(documents)
            }
