...
class AnomalyRecord:
    ...
    garbled_paragraphs: list[dict] = field(
        default_factory=list
    )  # [{"index": int, "position": int, "text": str}]
...
def _detect_word_freq_anomaly(text: str, threshold: int = 5) -> tuple[bool, list[str]]:
    ...
    # 只统计非空白、非标点字符
    content_chars = [
        c for c in text if c.strip() and c not in "，。！？：；、''（）【】《》…—·.,;:!?()[]{}/"
    ]
    if content_chars:
...
) -> tuple[list[Document], list[AnomalyRecord]]:
...
def detect_statistical_anomalies(
    documents: list[Document], prev_records: list[AnomalyRecord] | None = None
) -> tuple[list[Document], list[AnomalyRecord]]:
    ...
    if total_anomalies > 0:
        ...
        for rec in all_records:
            if rec.length_outlier:
                anomaly_types[f"length_{rec.length_outlier}"] = (
                    anomaly_types.get(f"length_{rec.length_outlier}", 0) + 1
                )
