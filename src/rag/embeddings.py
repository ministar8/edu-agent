...
def _embedding_timeout() -> httpx.Timeout:
    ...
    return httpx.Timeout(
        total, connect=min(5.0, total), read=total, write=total, pool=min(5.0, total)
    )
...
class OpenAICompatibleEmbeddings(BaseModel, Embeddings):
    ...
    def _embed_batch(self, texts: list[str]) -> list[list[float]]:
        ...
        with metrics.timer(
            "embedding_batch", stage="embeddings", tags={"model": self.model, "mode": "sync"}
        ) as mt:
            mt.update(
                {
                    "batch_size": len(texts),
                    "avg_text_chars": round(sum(original_lengths) / len(original_lengths), 3)
                    if original_lengths
                    else 0.0,
            ...
            if nan_indices:
                logger.warning(
                    "NaN detected in %d/%d embeddings, retrying individually",
                    len(nan_indices),
                    len(results),
                )
    ...
    async def _aembed_batch(
        ...
        with metrics.timer(
            "embedding_batch", stage="embeddings", tags={"model": self.model, "mode": "async"}
        ) as mt:
            mt.update(
                {
                    "batch_size": len(texts),
            ...
            if nan_indices:
                logger.warning(
                    "NaN detected in %d/%d async embeddings, retrying individually",
                    len(nan_indices),
                    len(results),
                )
                for i in nan_indices:
