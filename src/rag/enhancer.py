...
def enhance_documents(documents: list[Document]) -> list[Document]:
    for doc in documents:
        ...
        doc.metadata["source_type"] = (
            doc.metadata.get("source_type") or source_ext.lstrip(".") or "unknown"
        )
        ...
        doc.metadata["heading_slug"] = _build_heading_slug(
            str(doc.metadata.get("heading_path") or heading)
        )
        doc.metadata["has_heading"] = bool(heading)
        doc.metadata["has_code_block"] = (
            "```" in doc.page_content
            or "~~~" in doc.page_content
        ...
        doc.metadata["is_structured"] = content_type in {
            "section",
            "list",
            "exercise",
            "answer",
