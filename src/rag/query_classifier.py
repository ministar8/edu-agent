    ...
    def __repr__(self) -> str:
        skips = [
            ...
            if getattr(
                self,
                f"skip_{s}"
                if s not in ("meta", "lw_rerank")
                else ("skip_metadata_routes" if s == "meta" else "lightweight_rerank"),
            )
        ]
...
_classify_cache: BoundedCache[str, QueryCategory] = BoundedCache(
    max_size=_CLASSIFY_CACHE_MAX, name="classify"
)
...
def _classify_by_rules(normalized: str, terms: list[str]) -> QueryCategory:
    ...
    for category, markers in _RULE_MARKERS.items():
        if category == "code":
            ...
            flags["code"] = any(m in lowered for m in markers) or any(
...
async def _classify_by_llm(query: str) -> QueryCategory | None:
    ...
    超时/解析失败/API 错误一律由 call_structured 归一为 None → 降级回规则分类。
    """
    from rag.schemas import QueryClassifyResult
...
def _needs_llm_fallback(rule_cat: QueryCategory) -> bool:
    ...
    any_flag = any(
        [
            rule_cat.is_code,
            rule_cat.is_exercise,
            rule_cat.is_answer,
            rule_cat.is_structured,
            rule_cat.is_concept,
            rule_cat.is_comparison,
...
async def aclassify_query(query: str, terms: list[str] | None = None) -> QueryCategory:
    ...
    cached = _classify_cache.get(normalized)
    ...
    # 带 LLM 调用，用「取-算-存」：允许竞态重复分类，但不持锁等待 IO
    _classify_cache.set(normalized, rule_cat)
...
def classify_query(query: str, terms: list[str] | None = None) -> QueryCategory:
    ...
    # 仅规则分类（不调用 LLM，避免 asyncio.run 嵌套）；纯计算，允许竞态重复
    rule_cat = _classify_cache.get_or_compute(
        normalized, lambda: _classify_by_rules(normalized, terms)
    )
