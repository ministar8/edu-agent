...
SYNONYM_EXPAND_RE: re.Pattern[str] = (
    re.compile("|".join(re.escape(k) for k in _SYNONYM_EXPAND_KEYS), re.IGNORECASE)
    if _SYNONYM_EXPAND_KEYS
...
SYNONYM_NORM_RE = (
    re.compile("|".join(re.escape(k) for k in _SYNONYM_NORM_KEYS))
    if _SYNONYM_NORM_KEYS
    else re.compile(r"(?!)")
)
