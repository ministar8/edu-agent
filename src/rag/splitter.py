    ...
    for block in blocks:
        ...
        if block.startswith("__CODE_BLOCK_") and block.endswith("__"):
            ...
            if idx_match:
                units.append(
                    {
                        "text": code_blocks[int(idx_match.group(1))],
                        "is_code": True,
                        "is_qa": False,
                        "is_table": False,
                        "is_formula": False,
        ...
        if block.startswith("__TABLE_BLOCK_") and block.endswith("__"):
            ...
            if idx_match:
                units.append(
                    {
                        "text": table_blocks[int(idx_match.group(1))],
                        "is_code": False,
                        "is_qa": False,
                        "is_table": True,
                        "is_formula": False,
        ...
        if block.startswith("__FORMULA_BLOCK_") and block.endswith("__"):
            ...
            if idx_match:
                units.append(
                    {
                        "text": formula_blocks[int(idx_match.group(1))],
                        "is_code": False,
                        "is_qa": False,
                        "is_table": False,
                        "is_formula": True,
...
def _build_overlap_units(units: list[dict], chunk_overlap: int) -> list[dict]:
    ...
    for unit in reversed(units):
        if (
            unit.get("is_code")
            or unit.get("is_qa")
            or unit.get("is_table")
            or unit.get("is_formula")
        ):
            break
    ...
    **拆开**，其内部结构始终完整。反过来，若禁止合并，过短的原子 chunk 会连同
    ...
    for chunk in chunks[1:]:
        ...
        if (
            ...
            and not prev.get("is_qa")
            and not chunk.get("is_qa")
        ):
            prev["text"] = prev_text + "\n\n" + cur_text
        else:
...
def split_documents(
    ...
    for doc in documents:
        ...
        # ── 切分 chunk ──
        from rag.rag_utils import detect_content_type as _detect_content_type
        ...
        for i, section in enumerate(sections):
            ...
            heading_is_exercise = any(
                kw in section["heading"] for kw in ("题", "例题", "习题", "练习")
            )
            if has_answer_marker and (
                heading_is_exam or heading_is_exercise or content_type in ("exercise", "answer")
            ):
            ...
            parent_windows = [
                _build_parent_window_text(detail_chunks, ci) for ci in range(chunk_count)
            ]
    ...
    # ── 统计 ──
    chunk_lengths = [
        int(c.metadata.get("char_count") or len(c.page_content or "")) for c in valid_chunks
    ]
    incomplete_detail_count = sum(
        1 for c in valid_chunks if not _is_sentence_complete(c.page_content)
    )
    merged_qa_count = sum(
        1 for c in valid_chunks if c.metadata.get("section.chunk_role") == "merged_qa"
    )
    metrics.emit(
        ...
        values={
            ...
            "avg_chunk_chars": round(sum(chunk_lengths) / len(chunk_lengths), 3)
            if chunk_lengths
            else 0.0,
            "p50_chunk_chars": _percentile(chunk_lengths, 0.5),
            "p90_chunk_chars": _percentile(chunk_lengths, 0.9),
            "short_chunk_ratio": round(
                sum(1 for n in chunk_lengths if n < max(MIN_CHUNK_LENGTH * 3, 60))
                / len(chunk_lengths),
                6,
            )
            if chunk_lengths
            else 0.0,
            "long_chunk_ratio": round(
                sum(1 for n in chunk_lengths if n > 1600) / len(chunk_lengths), 6
            )
            if chunk_lengths
            else 0.0,
            "mid_sentence_cut_rate": round(incomplete_detail_count / len(valid_chunks), 6)
            if valid_chunks
            else 0.0,
        },
    )
