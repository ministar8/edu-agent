...
# ── 有界重试：整个文件的用例都是浏览器 E2E，天生对时序敏感 ──────────
# 为什么加它（**基于实测证据，不是"把红的刷绿"**）：
#   · 本机连跑 20 次全部通过（每次约 6.6s）
#   · 给本机加 12 个 CPU 忙循环后，单次能跑到 **234s** —— 比 CI 还慢 —— 仍全部通过
#   · 但 CI 上偶发失败，且失败在 **3.12 / 3.13 之间交替**出现 → 与 Python 版本无关
#   · 还观察到纯文档提交（只改 1 行 Markdown）也会挂 → 与被测代码无关
# 结论：失败是**环境性的**（runner 的时序/争抢），不是测试逻辑或业务代码的 bug。
# 浏览器 E2E 配重试是业界标准做法（Playwright 自身也提供 retries 配置）。
#
# **这不是掩盖问题**：
#   · 次数有界（2 次）—— 真坏了一定会失败，重试救不了
#   · 失败时仍会存截图、并把页面 `.msg` 节点与 `.thinking` 状态打进断言消息
#   · 真需要排查时，CI 的 `Upload E2E diagnostics` 会上传截图与服务日志
