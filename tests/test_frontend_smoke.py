...
# ── 关于「有界重试」的一段教训（别再踩）──────────────────────────
# 这里**曾经**加过 `pytestmark = pytest.mark.flaky(reruns=2, reruns_delay=5)`，
# 理由是「CI 失败是环境性时序问题，与代码无关」。**那个诊断是错的，已移除。**
#
# 当时的推理链是：本机连跑 20 次全绿 + 加压到 234s 仍全绿 → 判定为环境抖动。
# 但真正的失败在 `core/cache.py` 的淘汰 off-by-one（`_evict_locked` 的守卫
# 写成 `len <= max_size`，而调用方触发条件是 `len >= max_size`，两者在
# `len == max_size` 处错开一格）—— **与 E2E 毫无关系**。
# 定位它靠的是把失败用例的断言消息变成 GitHub 注解（见 pyproject 的
# pytest-github-actions-annotate-failures），而不是继续猜。
#
# **为什么把这段写在这里**：给整个文件加重试会让**以后真正的 flaky 更难被发现** ——
# 它会先被重试吞掉几次，掩盖成一个"偶尔红"的模糊信号。
# 加缓解手段之前必须先确认失败是**偶发**；「本机复现不了」不等于「偶发」。
#
# 上面 `_WAIT_*` 的放宽仍然保留 —— 那一条有独立依据：
# 实测 CI 的 Test 步骤要 72~94s，而本机整个 E2E 只要 6.6s，确实慢一个数量级。
